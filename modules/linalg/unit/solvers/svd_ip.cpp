...
//==============================================================================
#define NT2_UNIT_MODULE "nt2 linalg toolbox - svd_ip"
...
#include <nt2/toolbox/linalg/functions/solvers/solve_svd_ip.hpp>
...
NT2_TEST_CASE_TPL ( LUSolveIP, (double)) 
{
  ...
  std::cout << std::endl;
  nt2::solve_svd_ip_return<table_t, table_t, table_t> f = solve_svd_ip(a, x, b);
  std::cout <<  "a ->"<< std::endl; 
  for(std::size_t i = 1; i <= size(a, 1); i++)
    {
      for(std::size_t j = 1; j <= size(a, 2); j++)
