...
//==============================================================================
#define NT2_UNIT_MODULE "nt2 algebra toolbox - gemm_t"
...
NT2_TEST_CASE_TPL ( gemm_t, NT2_REAL_TYPES) 
{
  using nt2::meta::make_container; 
  using nt2::of_size_;
  using nt2::gemm_t;
  typedef typename make_container<nt2::tag::table_, T, of_size_<17,5>  >::type table_type_a;
  typedef typename make_container<nt2::tag::table_, T, of_size_<5,17>  >::type table_type_bt;
  ...
  std::size_t dim1_bt = size(bt)(1);
  std::size_t dim2_bt = size(bt)(2);
  T tmp;
  ...
  for(std::size_t i = 1; i <= dim1_bt; i++)
    for(std::size_t j = 1; j <= dim2_bt; j++)
    {
      pos1 = boost::fusion::make_vector(i,j);
      bt[pos1] = i*j;
    }
