...
//==============================================================================
#ifndef NT2_TOOLBOX_ALGEBRA_FUNCTIONS_BLAS_TRSV_HPP_INCLUDED
#define NT2_TOOLBOX_ALGEBRA_FUNCTIONS_BLAS_TRSV_HPP_INCLUDED
...
#define NT2_TRSV(T, PREFIX)                                             \
    ...
    {                                                                   \
      BOOST_PP_CAT(PREFIX,BOOST_PP_CAT(trsv,_))(uplo,trans,diag,n,a,lda,x,incx);       \
    }                                                                   \
        
    NT2_TRSV(double, d)
    NT2_TRSV(float,  s)
    NT2_TRSV(std::complex<double>, z)
    NT2_TRSV(std::complex<float>, c)
...
#undef NT2_TRSV
    ...
    /////////////////////////////////////////////////////////////////////////////
      NT2_FUNCTOR_IMPLEMENTATION( nt2::tag::trsv_, tag::cpu_
                                , (A0)(S0)(A1)(S1)(ST)
                                , (unspecified_ < ST > )
                                  ...
                                  )
    {
                                                ...
                                                )
      {
        ...
        //                           "Matrix-vector product y = A°*x° (trsv) must be called with unit-triangular table A");
        //         BOOST_ASSERT_MSG( (nt2::is_triangular(a, upload)), 
        //                           "Matrix-vector product y = A°*x° (trsv) must be called with triangular table A");
        BOOST_ASSERT_MSG( (nt2::is_square(a)), 
                          "Matrix-vector product y = A°*x° (trsv) must be called with square table A");

        const long int n   = nt2::extent(a)[0]; 

        BOOST_ASSERT_MSG( (n == nt2::numel(x)), //TODO replace numel by proper dim here
                          "In matrix-vector product y = A°*x° (trsv) the inner number of columns and lines \
        ...
        trsv(&upload,&transa,&diag,&n,a.begin(),&lda,x.begin(),&incx);
      }
    };
