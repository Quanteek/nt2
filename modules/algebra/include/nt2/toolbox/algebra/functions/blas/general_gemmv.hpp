...
{
      gemv(&transa,&m,&n,&alpha,a1.begin(),&lda,a2.begin(),&x,&beta,a0.begin(),&y);
...
#define NT2_GEMV(T, PREFIX)                                       \
  inline void gemv(const char *ta, const long int *m,             \
                   ...
                   T *y)                                          \
  {                                                               \
    F77NAME(PREFIX ## gemv)(ta,m,n,al,a,lda,x,be,y);              \
  }                                                               \
...
//       assert(is_vector(y));
      ...
      const long int n   = nt2::size(a1)(transa=='T'?1:2);
      const double alpha = al; 
      const long int lda = nt2::details::padding(a1);
      const double beta  = be; 
      gemv(&transa,&m,&n,&alpha,a1.begin(),&lda,a2.begin(),&x,&beta,a0.begin(),&y);
    }
  };
  
  template < char T,  class A0,  class A1,  class A2,  class A3,  class A4>
  inline void gemv(A0& a0, A1 const& a1, A2 const& a2,A3 const& a3,A4 const&  a4)
  {
    typedef typename A0::value_type value_type; 
    general_gemv(a0, a1, a2, T, a3, a4); 
  }
  template < char T,  class A0,  class A1,  class A2,  class A3>
  inline void gemv(A0& a0, A1 const& a1, A2 const& a2,A3 const& a3)
  {
    typedef typename A0::value_type value_type; 
    general_gemv(a0, a1, a2, T, a3, Zero<value_type>()); 
  }
  ...
  {
    typedef typename A0::value_type value_type; 
    general_gemv(a0, a1, a2, T, One<value_type>(), Zero<value_type>()); 
  }
