...
// NT2_TEST_CASE_TPL( cif_value_type, NT2_TYPES )
// {
//   using boost::mpl::_;
//   using nt2::meta::value_type_;

//   NT2_TEST_EXPR_TYPE( (nt2::cif(4,4))
//                      , (value_type_<_>)
//                      , (double)
//                      );
//   NT2_TEST_EXPR_TYPE( (nt2::cif(4, 4,nt2::meta::as_<T>()))
//                      , (value_type_<_>)
//                      , (T)
//                      );
//   NT2_TEST_EXPR_TYPE( (nt2::cif(nt2::of_size(4,4)))
//                     , (value_type_<_>)
//                     , (double)
//                     );

//   NT2_TEST_EXPR_TYPE( (nt2::cif(nt2::of_size(4,4),nt2::meta::as_<T>()))
//                     , (value_type_<_>)
//                     , (T)
//                     );
// }
 ...
 NT2_TEST_CASE( cif_nd_untyped )
 {
   ...
   for(int i=1;i<=8;++i) for(int j=1;j<=4;++j)NT2_TEST_EQUAL( double(j), double(x2(i, j)) );
   nt2::table<double> x3 = nt2::cif(2, 4);
   for(int i=1;i<=2;++i) for(int j=1;j<=4;++j)NT2_TEST_EQUAL( double(j), double(x3(i, j)) );
   nt2::table<double> x4 = nt2::cif(1, 4);
   for(int j=1;j<=4;++j)NT2_TEST_EQUAL( double(1), double(x4(1, j)) );
   nt2::table<double> x5 = nt2::cif(2, 1);
   for(int i=1;i<=2;++i) NT2_TEST_EQUAL( double(j), double(x5(i, 1)) );
   nt2::table<double> x6 = nt2::cif(nt2::of_size(8, 6));
   for(int i=1;i<=8;++i) for(int j=1;j<=6;++j)NT2_TEST_EQUAL( double(j), double(x6(i, j)) );
...
NT2_TEST_CASE( cif_expr )
{
  ...
  nt2::table<double> x1 = nt2::cif( nt2::size(a) );
  for(int i=1;i<=4;++i) for(int j=1;j<=5;++j) NT2_TEST_EQUAL(  double(j), double(x1(i, j)) );
...
NT2_TEST_CASE_TPL( cif_typed_expr, NT2_TYPES )
{
  ...
  for(int i=1;i<=3;++i) for(int j=1;j<=4;++j) NT2_TEST_EQUAL( T(j), T(x1(i, j)));
  nt2::table<int> a( nt2::of_size(4,5) );
  nt2::table<T> x2 = nt2::cif( nt2::size(a), nt2::meta::as_<T>() );
  for(int i=1;i<=4;++i) for(int j=1;j<=5;++j) NT2_TEST_EQUAL( T(j), T(x2(i, j)));
